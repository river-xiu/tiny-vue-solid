    ...
    generateBundle: (output: NormalizedOutputOptions, bundle: OutputBundle) => {
      ...
      for (const name of jsAssets) {
        ...
        code = code
          ...
          .replace(/from ".*\/vue-icon\/src\//g, 'from "./lib/')
          .replace(/vue-Icon/gi, 'solid-icon')
          .replace(/".*\.\/.*\/label-wrap"/, '"./label-wrap"')
          .replace(/".*\.\/.*\/tall-storage.vue"/, '"./tall-storage"')
          .replace(/@opentiny\/solid/g, '@opensolidtiny/solid')
...
// import IconLoading from "@opentiny/vue-icon/loading/index.ts";
